 ...
 * @notice Base contract for account recovery modules (Zyra architecture)
...
abstract contract BaseRecovery is IModule, EIP712 {
    ...
    bytes32 constant _RECOVERY_DATA_TYPEHASH =
        keccak256(
            "RecoveryData(address recoveringAddress,bytes newOwner,uint256 nonce)"
        );
    ...
    function executeRecovery(address recoveringAddress) external virtual {
        ...
        IZyra(recoveringAddress).resetOwners(recoveryState.newOwner);
    ...
    function _recoveryDataHash(
        RecoveryData calldata data
