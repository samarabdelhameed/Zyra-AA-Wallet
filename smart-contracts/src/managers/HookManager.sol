...
import {ZyraStorage} from "../libraries/ZyraStorage.sol";
 ...
 */
abstract contract HookManager is Auth, IZyra {
    ...
    function _addHook(bytes calldata hookAndData, bool isValidation) internal {
        ...
        IInitializable(hookAddress).init(initData);
    ...
    function _removeHook(address hook, bool isValidation) internal {
        ...
        (bool success, ) = hook.call(
            abi.encodeWithSelector(IInitializable.disable.selector)
        );
    ...
    {
        validationHooks = ZyraStorage.layout().validationHooks;
    }
    ...
    {
        executionHooks = ZyraStorage.layout().executionHooks;
    }
    ...
    {
        hookDataStore = ZyraStorage.layout().hookDataStore;
    }
